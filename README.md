...
1. 拡大率・スクロール位置・表領域の高さはモジュールごとに記憶されます。記憶が残るのは**プレビューパネルを開いている間**で、直近 20 モジュールまでが対象です。同じモジュールへ切り替え直すと復元されます。プレビューパネルを閉じると記憶は失われ、次に開いたときは初期状態から始まります。
...
- コメントにせず `#` を文字として使いたい場合は `\#` と記述します（`\` は出力されません）
