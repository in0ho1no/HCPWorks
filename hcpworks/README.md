...
`.hcp` ファイルの編集中、括弧やクォートを入力すると対となる閉じ括弧が自動補完されます。  
また、テキストを選択した状態で括弧を入力すると、選択範囲が括弧で囲まれます。  
...
`.hcp` ファイルで `\module` および `\table` ブロックの折り畳みをサポートします。  
エディタ左端のガター（行番号横）に表示される `▼` アイコンをクリックすると折り畳めます。

| キーワード | 折り畳み範囲 |
| --- | --- |
| `\module` | 次の `\module` が来るまでの行（末尾の空行を除く） |
| `\table` | 空行・`\data`・次の `\table`・`\module` のいずれかが来るまでの行 |
...
\table | 表の記載 | \moduleから\dataの間に記載すること。csv形式で、連続するカンマは1つにまとめる。<br>`\table 名前` でキャプションを付けられる。<br>行頭インデント(タブ/半角4スペース=1階層)で構造体メンバのような親子階層を表現できる。<br>セル内で改行したい場合は `<br>` を使う(Excelへは書式あり貼り付けでセル内改行になる)。<br>セル内でも `<del>` / `<ins>` による文字列装飾を使える。<br>画像の出力には含めない。

### レベル0以上に記載できる表記

表記 | 内容 | 注意点
---| --- | ---
\data | モジュール内で利用するデータの定義 | 重複させないこと。重複した場合、一番最初に登場する文字列のみ描画する。<br>`\data (テキスト)` のように括弧で囲んだテキストを指定すると、データ列にグレーの補足テキストとして表示される。
\fork | 条件分岐 | -
...
`\data` と `\in` / `\out` のデータ名照合では、`<ins>` / `<del>` 装飾タグの有無を区別しません。例えば `\data <ins>カウンタ</ins>` と `\out カウンタ` は同一データとして接続されます。
...
行中の文字列の一部を装飾できます。`\mod` などの行種別や `\table` のセル内で使えます。<br>各タグは対(`<del></del>` / `<ins></ins>`)で使用します。タグの入れ子・別タグの混入・対応しない開閉は記法エラーとして赤背景で表示します。
